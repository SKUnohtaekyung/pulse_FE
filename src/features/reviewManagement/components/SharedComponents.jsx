            ...
            <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
                ...
                <div className="flex gap-1 bg-neutral-100 p-1 rounded-xl w-full sm:w-fit">
                    <button
                        ...
                        className={`flex-1 sm:flex-initial px-4 sm:px-6 py-2.5 rounded-lg font-medium transition-all ${activeTab === 'ai-generate'
                    ...
                    <button
                        ...
                        className={`flex-1 sm:flex-initial px-4 sm:px-6 py-2.5 rounded-lg font-medium transition-all ${activeTab === 'saved-templates'
                                ? 'bg-white text-neutral-900 shadow-sm'
                                : 'text-neutral-600 hover:text-neutral-900'
                            }`}
                    >
 ...
 * - 선택적 액션 버튼 지원
 */
export function Toast({ message, onClose, action }) {
    useEffect(() => {
 ...
 * - 드롭다운으로 개수 변경 가능
 */
export function GenerateButton({ onGenerate, isLoading }) {
                            ...
                            <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-lg border border-neutral-200 py-2 z-20 min-w-[80px]">
                                {[1, 3, 5].map((num) => (
                                    <button
                                        ...
                                        className={`w-full px-4 py-2 text-left hover:bg-neutral-50 transition-colors ${count === num ? 'text-[#002B7A] font-semibold' : 'text-neutral-700'
                                            }`}
                                    >
                                        {num}
                                    </button>
