 ...
 * - 저장/취소 액션
 */
export function SaveTemplateModal({ onClose, onSave }) {
    const [name, setName] = useState('감사 + 재방문 (친근함)');
    const [selectedCategories, setSelectedCategories] = useState(['감사']);
    const [selectedTags, setSelectedTags] = useState(['긍정', '이모지']);
 ...
 * - 복사, 수정, 삭제 기능
 */
export function SavedTemplatesTab({ templates, onDelete }) {
