...
*   **Dashboard Layout:** 상권 분석 페이지와 통합 분석 페이지의 라우팅 충돌 해결.
*   **Dashboard Home:** 병합 충돌로 인한 차트 컨테이너 및 버튼 텍스트 오류 수정.
...
*   **Backend Interface Readiness (백엔드 실연동 준비):**
    *   **Raw File Capture:** 프론트엔드용 Blob URL뿐만 아니라 실제 업로드용 File Object를 별도 상태(`selectedFile`)로 관리.
...
*   **Weather Optimized:** 날씨 및 이벤트 정보를 좌측 상단으로 컴팩트하게 정리하고, AI 홍보 제안 배너를 헤드라인 우측으로 이동하여 공간 효율 극대화.
...
*   **3D Interactive Core:** Three.js를 활용하여 살아 움직이는 듯한 Fluid Core를 구현하고, 마우스 움직임에 반응하는 인터랙티브 경험 제공.
*   **Split-Screen Layout:**
    *   **Visual Balance:** 3D 오브젝트를 좌측, 정보 입력을 우측에 배치하는 2분할 레이아웃으로 전면 개편하여 시각적 간섭 제거.
    *   **Focus:** 텍스트와 버튼의 가독성을 극대화하고, 로딩 완료 시 자연스럽게 "시작하기"로 시선을 유도.
*   **Premium UI Detail:**
    *   **Glassmorphic CTA:** "PULSE 시작하기" 버튼에 글래스모피즘 & 샤인(Shine) 애니메이션을 적용하여 클릭 유도 강화.
...
## 2026-01-25 (Unified Insight & AI Chatbot)

### 📊 Unified Insight Page Redesign (분석 페이지 통합 개편)
*   **Master-Detail Layout (2-Pane Structure):**
    *   기존에 분리되어 있던 '단골 손님 분석'과 '상권 분석'을 하나의 페이지(`UnifiedInsightPage`)로 통합했습니다.
...
*   **Header Integration:** 챗봇을 별도의 플로팅 위젯에서 헤더(`Header`)의 툴바 요소로 통합했습니다.
...
*   **UI/UX Polish (디자인 및 사용성 개선):**
    *   **Style Gallery Redesign:** 카드 높이 확대(200px) 및 텍스트 줄바꿈 허용으로 가독성 확보.
    *   **Context-Aware Design:** 선택된 스타일에 따라 테마 색상(Amber, Purple, Rose)이 은은하게 적용.
...
*   **Local Analysis Section (우리 동네 상권 분석):**
    *   **Peak Time Chart:** 그래프 영역의 왼쪽 여백(`pl-10`)을 늘려 키워드 리스트와의 간격 확보, 오른쪽 여백(`pr-10`)을 줄여 그래프 확장.
    *   **Header Alignment:** "상권 심층 분석" 타이틀과 설명 문구 하단 정렬(`items-end`)로 배치.
    *   **CTA Button:** 홍보 영상 제작 버튼의 호버 시 위로 올라가는 효과 제거.
*   **Customer Analysis Section (단골 손님 유형 분석):**
    *   **Text & Logic Updates:** "AI 컨설턴트" → "~ 유형 분석중"으로 동적 변경. 기본 메시지 및 헤더 설명 문구 개선.
    *   **Layout & Navigation:** 탭 버튼 위치를 사이드바 네비게이션과 맞춤(`mt-4`). 상단 타이틀 복구 및 버튼 크기 조정.

### 🛠 Dashboard Refactoring (대시보드 리팩토링)
*   **Component Extraction:** `WeatherAnimation`, `WidgetHeader` 컴포넌트 분리.
*   **Code Optimization:** `WIDGET_BASE_CLASSES` 상수 도입. `DashboardHome.jsx` 중복 코드 제거.
...
*   **Layout Standardization:** 모든 입력 폼의 너비를 520px로 통일, Flex Layout 비율 조정.
*   **Visual Refinements:** 입력창 호버 인터랙션, 정렬 및 여백 최적화.
*   **Code Optimization:** `AuthPage.css` 전면 재작성.

## 2025-12-01

### 🎨 Auth UI Redesign (로그인/회원가입 UI 개편)
*   **Split Screen Layout:** 좌측 3D 비주얼 영역(60%)과 우측 입력 폼 영역(40%)으로 분할된 모던 레이아웃 적용.
*   **3D Background:** React Three Fiber를 활용한 인터랙티브 3D 파티클 및 지오메트릭 애니메이션 배경 구현.
...
*   **Navigation Logic:** 대시보드 및 분석 페이지에서 홍보 영상 제작 페이지로 이동 시, 문맥 데이터(`title`, `vibe`) 전달 기능 구현.
*   **State Management:** `DashboardLayout`에서 네비게이션 파라미터 관리.
*   **VEO3 Payload Logic:** `VideoCreator.jsx` 리팩토링, `generateVeoPayload` 함수 구현.
*   **CTA Updates:** 대시보드 홈, 손님 분석, 상권 분석 페이지의 데이터 연동.
...
*   **Project Structure:** `test.jsx` 단일 파일을 Vite + React + Tailwind CSS 구조로 분리 및 재구성.
    *   `src/components/layout`, `src/features`, `src/styles` 등 디렉토리 구조화.
*   **Data Extraction:** `mockData.jsx` 및 `constants/index.js`로 데이터와 상수 분리.

### 🎨 Design Updates (디자인 수정)
*   **Sidebar Layout:** 로고 영역과 네비게이션 영역 분리, 확장 시 아이콘 위치 고정.
*   **Responsive Scaling:** 1920px 해상도 기준 125% 확대 비율 유지.
*   **Insight Page UX Refinement:** 방문 여정(좌측)과 프로필/액션(우측) 위치 교체, 프로필 탭 통합.
*   **Dashboard Redesign:** No Scroll Policy 적용, 2-Column 구조(9:3), 위젯 최적화.
*   **My Page Redesign:** 2x2 Grid 카드 배치, No Scroll Policy 적용.

### 📘 Documentation (문서화)
*   **Design Guide:** '4. Layout Principles' 섹션 추가 (No Scroll Policy 명시).

### 🐛 Bug Fixes (버그 수정)
*   **Syntax Errors:** `PersonaSection.jsx` 중복 태그 및 `globals.css` 문법 오류 수정.
...
*   **Design System Standardization:** Border Radius, Sidebar, Typography 등 디자인 시스템 표준화.
*   **Assets & Branding:** `PULSE_LOGO.png` 교체 및 파비콘 업데이트.
*   **Global Styles:** `overflow-hidden` 강제 적용, 새로운 컬러 팔레트 적용.

### 🐛 Bug Fixes & Refactoring (버그 수정 및 리팩토링)
*   **Syntax Errors:** `SuggestionChip` 컴포넌트 닫는 태그 누락 수정.
