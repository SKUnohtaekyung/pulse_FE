...
* **Data Collection:** **Playwright** (네이버/카카오맵 리뷰 크롤링) & **Kakao Local API** (상권/키워드 검색)
* **NLP & Analysis:**
    * **Kiwi:** 한국어 형태소 분석
    * **BERTopic:** 리뷰 주제 군집화 (Clustering)
* **Generative AI:** **LLM API (Gemini / GPT)** (키워드 기반 페르소나 및 인사이트 생성)
* **Video Processing:** **Google VEO3 API** (프롬프트 및 페이로드 전송 기반 숏폼 생성)
...
2.  **프롬프트 페이로드 생성:** API 요청 및 이미지 분석을 통해 VEO3가 이해할 수 있는 JSON 형태의 프롬프트(동작, 앵글, 분위기 명세)로 변환합니다.
3.  **VEO3 API 렌더링 (Rendering):**
    * FastAPI가 변환된 페이로드와 이미지를 **Google VEO3 API**로 전송하여 고품질 영상을 렌더링합니다.
4.  **MP4 Output:** 최종 결과물인 `.mp4` 파일의 URL을 받아옵니다.
...
| **BE ↔ AI** | `POST` | `/internal/render` | AI 서버에 영상 렌더링 페이로드 전송 명령 |
| **BE/AI ↔ EXT** | `GET` | Kakao Local API | 키워드 및 상권 정보 검색 |
| **BE/AI ↔ EXT** | `POST` | Google VEO3 API | 텍스트/이미지 기반 숏폼 렌더링 요청 |