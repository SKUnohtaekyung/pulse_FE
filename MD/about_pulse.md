 ...
 **주변 상권 및 손님 분석 → 홍보 영상 만들기 → 가게 현황 → 인플루언서 협찬(Pro)**
...
- 주변 상권 및 손님 분석 (리뷰 입력 기반, 상권 패턴 분석)
- 홍보 영상 만들기 (사진 업로드 → 템플릿 기반 mp4 생성)
- 가게 현황 (사용자 입력 기반 성과/행동 제안)
- 리뷰 관리 및 인플루언서 매칭 (Pro UI 기반)
...
여기에 Pro 요금제를 통해 고품질 마케팅이 필요한 사장님을 위해
**리뷰 관리 및 인플루언서 협찬 매칭** 기능이 제공된다.
...
# **4.1 주변 상권 및 손님 분석 (Guest & Market Insight)**

### *온라인 리뷰 및 지역 상권 데이터를 활용한 고객 니즈 요약 기능*

**주변 상권 및 손님 분석**은 외식업 사장님이 복잡한 분석 과정 없이
...
분석 기능은 **두 가지 분석 관점을 분리/통합된 화면**에서 제공한다.
...
- **From 가게 현황:** "퇴근길 직장인 타겟" (Vibe: **에너지**) → **영상 제목과 분위기 자동 세팅**
- **From 손님 분석:** "단골 손님 시그니처 메뉴" (Vibe: **무드**) → **영상 제목과 분위기 자동 세팅**
- **From 주변 상권 분석:** "우리 동네 핫플레이스" (Vibe: **프리미엄**) → **영상 제목과 분위기 자동 세팅**
...
# **4.3 가게 현황 (Store Dashboard)**

### *숫자를 보는 것이 아니라, “다음 행동”을 보여주는 행동 중심 대시보드*

**가게 현황**은 SNS 운영 초보자인 외식업 사장님이
...
바쁜 사장님을 위해, 긍정/부정 리뷰를 자동으로 분류하고 적절한 답글을 AI가 제안해주는 핵심 기능이다.
감정 분석을 통해 고객의 숨은 의도를 파악하고, 진정성 있는 소통을 돕는다. (기본 모델 제공, Pro에서 고도화)
...
## **① 주변 상권 및 손님 분석 (Understand)**

**"우리 가게와 상권을 이해하고, 마케팅 방향을 결정하는 단계"**

**Input**

- 온라인 리뷰 데이터
- 지역 상권 정보 (카카오 로컬 API 연동)
...
```mermaid
graph LR
    A[주변 상권 및 손님 분석] -->|타겟/분위기 전달| B[홍보 영상 만들기]
    B -->|영상 업로드| C[가게 현황]
    C -->|다음 행동 제안| B
...
| 대시보드 홈(가게 현황) | 루프의 시작 & 성과 확인 | • AI 제안 (Hero): 가장 시급한 행동 가이드• 매장 브리핑: 날씨/시즌 기반 팁• 상세 분석: 주간 성과 그래프, 트렌드 위젯 |
| 주제별 분석 (주변 상권/손님) | 전략 수립 | •  UI 분리: 주변 상권 특화(Macro) / 손님 페르소나(Micro) 특화 뷰• 플로팅 AI 채팅• CTA: "이 손님으로 영상 만들기" |
| 홍보 영상 만들기 | 콘텐츠 제작 | • 입력: 사진 업로드, 타겟/분위기 선택• 기획: AI 스토리보드 확인 및 수정• 결과: 숏폼 영상 프리뷰 및 다운로드 |
| 리뷰 관리 & 답변 | 고객 소통 | • 긍·부정 리뷰 분류, AI 답글 초안 생성 |
...
```mermaid
graph TD
    A[주변 상권 및 손님 분석] -->|전략 수립| B[홍보 영상 만들기]
    B -->|실행| C[가게 현황]
    C -->|피드백 & 제안| B
...
## 8.1 리뷰 분석 고도화 (Advanced Review Insights)

(현재 통합 리뷰 관리 및 답글 작성 기능은 코어로 편입됨)
향후 PULSE는 리뷰 분석 기능을 더욱 고도화하여 정밀한 타겟 마케팅으로 연결한다.

### 주요 기능

- 부정 리뷰 방어율 분석
- 리뷰에서 추출된 키워드 기반 릴스 주제 자동 할당
- 마이크로 트렌드 변화 감지
...
| 분류 | 기능 | 목적 |
| --- | --- | --- |
| **지속사용 기반 기능** | 리뷰 분석 고도화 / 사진 보관함 / PULSE Score | 사장님이 자주 방문할 이유 제공 |
